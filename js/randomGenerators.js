...
window.MathsGenerators = {
    ...
    if (t === 1) {
      ...
      if (subType === 'quot_neg') {
        ...
        return {
          ...
          hint1: `Garde la fraction entière : $E = \\frac{10^{${a}} \\times 10^{${b}}}{10^{${c}}} = \\frac{10^{${a + b}}}{10^{${c}}} = 10^{${a + b} - (${c})}$.`,
      ...
      return {
        ...
        hint2: `Garde la fraction complète : $D = \\frac{10^{${p * q}} \\times 10^{${r}}}{10^{${s}}} = \\frac{10^{${p * q + r}}}{10^{${s}}} = 10^{${p * q + r} - (${s})}$.` ,
        solution: `$$D = \\frac{(10^{${p}})^{${q}} \\times 10^{${r}}}{10^{${s}}} = \\frac{10^{${p * q}} \\times 10^{${r}}}{10^{${s}}} = \\frac{10^{${p * q + r}}}{10^{${s}}} = 10^{${p * q + r} - (${s})} = 10^{${finalExp}}$$`
      };
    }
  },
    ...
    if (t === 1) {
      ...
      if (subType === 'add_mult') {
        ...
        return {
          ...
          answer: sD === 1 ? String(sN) : `${sN}/${sD}`,
          placeholder: "Ex: 1",
          hint1: `Effectue la division en gardant toute l'expression : $D = \\frac{${a}}{${b}} \\times \\frac{${d}}{${c}} + \\frac{${e}}{${f}}$.`,
          solution: `$$D = \\frac{${a}}{${b}} \\div \\frac{${c}}{${d}} + \\frac{${e}}{${f}} = \\frac{${a}}{${b}} \\times \\frac{${d}}{${c}} + \\frac{${e}}{${f}} = \\frac{${a * d}}{${b * c}} + \\frac{${e}}{${f}} = \\frac{${a * d * f} + ${e * b * c}}{${b * c * f}} = ${this.formatFraction(numTotal, denTotal)}$$`
      };
    }
    } else {
      const configs = [
        { a: 2, b: 3, c: 1, d: 4, e: 5, f: 6, g: 1, h: 2, ansNum: 5, ansDen: 16 },
        { a: 3, b: 2, c: 1, d: 5, e: 1, f: 4, g: 2, h: 5, ansNum: 2, ansDen: 1 },
        { a: 4, b: 5, c: 1, d: 2, e: 2, f: 3, g: 1, h: 6, ansNum: 9, ansDen: 25 }
      ];
      const cfg = this.randChoice(configs);
      const topN = cfg.a * cfg.d - cfg.c * cfg.b;
      const topD = cfg.b * cfg.d;
      const bottomN = cfg.e * cfg.h + cfg.g * cfg.f;
      const bottomD = cfg.f * cfg.h;
      const finalN = topN * bottomD;
      const finalD = topD * bottomN;
      return {
        chapterId: '4N2',
        tier: 4,
        ...
        placeholder: "Ex: 5/16",
        hint1: "Mets les fractions au même dénominateur en gardant l'égalité complète.",
        solution: "$$F = \\frac{\\frac{" + cfg.a + "}{" + cfg.b + "} - \\frac{" + cfg.c + "}{" + cfg.d + "}}{\\frac{" + cfg.e + "}{" + cfg.f + "} + \\frac{" + cfg.g + "}{" + cfg.h + "}} = \\frac{\\frac{" + (cfg.a * cfg.d) + " - " + (cfg.c * cfg.b) + "}{" + topD + "}}{\\frac{" + (cfg.e * cfg.h) + " + " + (cfg.g * cfg.f) + "}{" + bottomD + "}} = \\frac{\\frac{" + topN + "}{" + topD + "}}{\\frac{" + bottomN + "}{" + bottomD + "}} = \\frac{" + topN + "}{" + topD + "} \\div \\frac{" + bottomN + "}{" + bottomD + "} = \\frac{" + finalN + "}{" + finalD + "} = " + this.formatFraction(finalN, finalD) + "$$"
      };
    }
  },
    ...
    if (t === 1) {
      ...
      if (subType === 'prod_10') {
        ...
        return {
          ...
          answer: String(ansExp),
          placeholder: `Ex: ${ansExp}`,
          hint1: `Conserve la fraction dans les étapes : $\frac{10^{${a}} \times 10^{${b}}}{10^{${c}}} = \frac{10^{${a + b}}}{10^{${c}}} = 10^{${a + b} - ${c}}$.`,
      ...
      return {
        ...
        hint1: "Applique les règles des puissances dans la fraction entière, puis écris le résultat en notation scientifique.",
        solution: `$$C = ${cfg.expr} = ${cfg.finalSci}$$\nL'exposant est donc : $$k = ${cfg.ansSciExp}$$`
      };
    }
  },
  ...
  generate5N1(tier = 1, mastery = 0) {
    ...
    if (t === 1) {
      ...
      if (subType === 'add_mult') {
        ...
        const u = this.randInt(12, 96);
        const div = this.randInt(2, 9);
        const m1 = this.randInt(2, 9);
        const m2 = this.randInt(2, 9);
        const a = this.randInt(8, 75);
        const b = this.randInt(2, 12);
        const c = this.randInt(2, 9);
        const d = this.randInt(2, 9);
        const variants = [
          { expr: `${u} - ${div * this.randInt(2, 10)} \\div ${div}`, ans: "Une différence", lastOp: "la soustraction", reason: "la division est prioritaire, la soustraction est faite en dernier" },
          { expr: `${a} + ${m1} \\times ${m2}`, ans: "Une somme", lastOp: "l'addition", reason: "la multiplication est prioritaire, l'addition est faite en dernier" },
          { expr: `(${a} - ${b}) \\times ${c}`, ans: "Un produit", lastOp: "la multiplication", reason: "la parenthèse est prioritaire, la multiplication est faite en dernier" },
          { expr: `(${c * d} + ${u}) \\div ${c}`, ans: "Un quotient", lastOp: "la division", reason: "la parenthèse est prioritaire, la division est faite en dernier" }
        ];
      ...
      const d = this.randInt(2, 8);
      const q = this.randInt(5, 18);
      const innerMul = this.randInt(3, 9);
      const innerSub = this.randInt(2, 12);
      const inner = innerMul * this.randInt(2, 6) - innerSub;
      const bracketStart = inner + d * this.randInt(2, 10);
      const left = (bracketStart - inner) / d;
      const m = this.randInt(2, 8);
      const p = this.randInt(3, 10);
      ...
      const cfg = {
        ...
        step1: `${innerMul} \\times ${Math.round((inner + innerSub) / innerMul)} - ${innerSub} = ${inner}`,
        step2: `${bracketStart} - ${inner} = ${bracketStart - inner}`,
        step3: `${bracketStart - inner} \\div ${d} = ${left}`,
        step4: `${p} - ${r} = ${right}`,
        step5: `${left} + ${m} \\times ${right} + ${add} = ${ans}`,
        ans
      };
    ...
    if (t === 1) {
      ...
      if (subType === 'missing_num') {
        ...
        return {
          ...
          solution: `On compare les deux termes de la fraction : $${n} ${n < d ? '<' : (n > d ? '>' : '=')} ${d}$, donc $\\frac{${n}}{${d}} ${n < d ? '<' : (n > d ? '>' : '=')} 1$.`
        };
      }
    } else if (t === 2) {
        ...
        const cases = [
          {
            ...
            ans: 5,
            steps: "$$C = \\frac{6^2 + 8^2}{5 \\times 2^2} = \\frac{36 + 64}{5 \\times 4} = \\frac{100}{20} = 5$$"
          },
          {
            expr: '\\frac{5^2 - 3^2}{2^3}',
            ans: 2,
            steps: "$$C = \\frac{5^2 - 3^2}{2^3} = \\frac{25 - 9}{8} = \\frac{16}{8} = 2$$"
          },
          {
            expr: '\\frac{10^2 - 2^6}{3 \\times 2^2}',
            ans: 3,
            steps: "$$C = \\frac{10^2 - 2^6}{3 \\times 2^2} = \\frac{100 - 64}{3 \\times 4} = \\frac{36}{12} = 3$$"
          },
          {
            expr: '\\frac{7^2 - 1}{2^4}',
            ans: 3,
            steps: "$$C = \\frac{7^2 - 1}{2^4} = \\frac{49 - 1}{16} = \\frac{48}{16} = 3$$"
          },
          {
            expr: '\\frac{3^4 - 1}{2 \\times 5}',
            ans: 8,
            steps: "$$C = \\frac{3^4 - 1}{2 \\times 5} = \\frac{81 - 1}{10} = \\frac{80}{10} = 8$$"
          },
          {
            expr: '\\frac{4^3 - 4}{2 \\times 3}',
            ans: 10,
            steps: "$$C = \\frac{4^3 - 4}{2 \\times 3} = \\frac{64 - 4}{6} = \\frac{60}{6} = 10$$"
          }
        ];
        const item = this.randChoice(cases);
        return {
          ...
          answer: String(item.ans),
          placeholder: `Ex: ${item.ans}`,
          hint1: "Le trait de fraction agit comme des parenthèses : calcule en gardant la fraction entière à chaque étape.",
  ...
  generate4A1(tier = 1, mastery = 0) {
    ...
    if (t === 1) {
      if (Math.random() < 0.5) {
        ...
        const x = this.randInt(2, 24);
        const m = this.randInt(2, 9);
        const a = this.randInt(3, 35);
        const res = x * m + a;
        return {
          chapterId: '4A1',
          tier: 1,
      ...
      const init = this.randInt(5, 60);
      const add = this.randInt(4, 35);
      return {
        chapterId: '4A1',
        tier: 1,
      ...
      if (Math.random() < 0.5) {
        ...
        const x = this.randChoice([-24, -18, -13, -9, -5, -2, 2, 3, 7, 11, 16, 23]);
        const a = this.randInt(2, 12);
        const b = this.randInt(3, 35);
        const res = (x * a) - b;
        return {
          chapterId: '4A1',
          tier: 2,
      ...
      const limit = this.randInt(8, 45);
      const n = this.randInt(3, limit + 18);
      const thenValue = this.randInt(12, 90);
      const elseValue = this.randInt(2, 55);
      const result = n > limit ? thenValue : elseValue;
      return {
        chapterId: '4A1',
        tier: 2,
        title: "Test conditionnel Si / Sinon dans Scratch (4ème)",
        statement: `Bloc Scratch : « Si N > ${limit} alors mettre R à ${thenValue} sinon mettre R à ${elseValue} ».\nSi la variable N vaut $${n}$, quelle sera la valeur de R ?`,
        type: "exact",
        answer: String(result),
        placeholder: `Ex: ${result}`,
        hint1: `Teste la condition $${n} > ${limit}$, puis choisis la valeur de R dans la branche exécutée.`,
        hint2: `Comme $${n} ${n > limit ? '>' : '\\le'} ${limit}$, c'est la branche « ${n > limit ? 'alors' : 'sinon'} » qui s'exécute.`,
        solution: `Comme $${n} ${n > limit ? '>' : '\\le'} ${limit}$, la branche ${n > limit ? '« alors »' : '« sinon »'} s'exécute et $R = ${result}$.`
      };
    } else if (t === 3) {
      if (Math.random() < 0.5) {
        // Retrouver le nombre de départ 4ème
        const start = this.randChoice([-18, -11, -7, -4, -2, 2, 3, 6, 9, 14, 21]);
        const a = this.randInt(2, 9);
      ...
      const x = this.randInt(-18, 28);
      const threshold = this.randInt(-8, 14);
      const multiplier = this.randInt(2, 9);
      const increment = this.randInt(6, 35);
      const result = x > threshold ? multiplier * x : x + increment;
      return {
        chapterId: '4A1',
        tier: 3,
        title: "Programme avec variable et condition (4ème)",
        statement: `Un script demande un nombre $x$. Si $x > ${threshold}$, il affiche $${multiplier} \\times x$, sinon il affiche $x + ${increment}$.\nPour $x = ${x}$, qu'affiche le lutin ?`,
        type: "exact",
        answer: String(result),
        placeholder: `Ex: ${result}`,
        hint1: `Teste la condition $${x} > ${threshold}$ pour déterminer la branche à suivre.`,
        hint2: x > threshold ? `La condition est vraie : calcule $${multiplier} \\times ${x} = ${result}$.` : `La condition est fausse : calcule $${x} + ${increment} = ${result}$.`,
        solution: `La condition $${x} > ${threshold}$ est ${x > threshold ? 'vraie' : 'fausse'} ; le lutin affiche ${x > threshold ? `$${multiplier} \\times ${x} = ${result}$` : `$${x} + ${increment} = ${result}$`}.`
      };
    } else {
      if (Math.random() < 0.5) {
        // Défi Scratch avec carré et distributivité
        const x = this.randInt(2, 28);
        const a = this.randInt(1, 18);
        const res = (x + a) * (x + a);
        return {
          chapterId: '4A1',
          tier: 4,
      ...
      const pas = this.randInt(3, 16);
      const seuil = this.randInt(18, 95);
      let val = 0;
      while (val < seuil) {
        val += pas;
      }
   ...
   */
  generateForChapter(chapterId, tier = null, mastery = null, _variationAttempt = 0) {
