...
# -*- coding: utf-8 -*-
"""
Générateur de Site/js/data/courses.js
Contient les fiches de cours synthétiques, formules encadrées, méthodes types et pièges à éviter pour les 18 chapitres de 3ème.
"""
...
OUTPUT_FILE = os.path.join(BASE_DIR, "Site/js/data/courses.js")
...
COURSES_DATA = {
    ...
    "G1": {
        ...
        "keyPoints": [
            {
                "title": "1. Configurations et Égalité des 3 rapports",
                "content": "Soit deux droites $(d)$ et $(d')$ sécantes en $A$. Soient $B, M \\in (d)$ et $C, N \\in (d')$.\nSi les droites $(MN)$ et $(BC)$ sont parallèles, alors :\n$$\\frac{AM}{AB} = \\frac{AN}{AC} = \\frac{MN}{BC}$$\n*(Fonctionne en configuration emboîtée ou en configuration papillon).*"
            },
            {
                "title": "2. Réciproque de Thalès",
                "content": "Si les points $A, M, B$ d'une part et $A, N, C$ d'autre part sont **alignés dans le même ordre**, et si :\n$$\\frac{AM}{AB} = \\frac{AN}{AC}$$\nalors les droites $(MN)$ et $(BC)$ sont parallèles."
            }
        ],
        "methods": [
            {
                "title": "Méthode : Calcul d'une longueur avec le produit en croix",
                "example": "On a $\\frac{4}{10} = \\frac{MN}{15}$. Calculer $MN$.",
                "steps": [
                    "**Étape 1** : Appliquer le produit en croix : $10 \\times MN = 4 \\times 15$.",
                    "**Étape 2** : Isoler $MN$ : $MN = \\frac{4 \\times 15}{10} = \\frac{60}{10} = 6\\text{ cm}$."
                ]
            }
        ],
        "traps": [
...
with open(OUTPUT_FILE, "w", encoding="utf-8") as fp:
    fp.write("// Fiches de cours de référence, définitions, formules et méthodes types pour les 18 chapitres\n")
    fp.write("window.MATHS_COURSES = ")
    json.dump(COURSES_DATA, fp, ensure_ascii=False, indent=2)
    fp.write(";\n")

print(f"Génération de courses.js terminée ({len(COURSES_DATA)} chapitres).")
